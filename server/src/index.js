...
app.get('/', (req, res) => {
  res.json({ status: 'ok', message: 'Pokemon Trading API', routes: ['/api/users/signup', '/api/users/login', '/api/users/:username', '/api/cards/add'] });
});
...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    const username = (req.body.username || req.body.requesterUsername || 'public').trim().toLowerCase();
...
app.post('/api/cards/add', upload.single('image'), async (req, res) => {
  try {
    ...
    const detectedName = providedName && providedName.trim()
      ? providedName.trim()
    ...
    saveUser(user);
    const totalValue = user.cards.reduce((sum, c) => sum + (Number(c.price) || 0), 0);
...
app.get('/api/trades/:username', (req, res) => {
  ...
  res.json({ trades: (user.trades || []).filter(t => t.to === uname && t.status === 'pending') });
});

app.post('/api/trades/create', upload.single('offeredImage'), async (req, res) => {
  try {
    const { ownerUsername, requesterUsername, ownerCardId, offeredCardId, offeredPrice, offeredCondition, offeredName } = req.body;
    ...
    if (offeredCardId) {
      ...
      if (!existingCard) return res.status(404).json({ error: 'Offered card not found in your collection' });
...
app.post('/api/trades/:username/:tradeId/accept', (req, res) => {
  ...
  if (trade.status !== 'pending') return res.status(400).json({ error: 'Trade already handled' });
  ...
  if (ownerCardIdx === -1) return res.status(404).json({ error: 'Owner card missing' });
